...
const char __wf_rom LOG_MEMORY_DATA_READ[] = "Read  0x%X - Address 0x%03X - PC = 0x%04X\n";
const char __wf_rom LOG_MEMORY_DATA_WRITE[] = "Write 0x%X - Address 0x%03X - PC = 0x%04X\n";
const char __wf_rom LOG_INTERRUPT_TRIGGERED[] = "Interrupt %s (%u) triggered\n";
...
static u4_t get_io(u12_t n)
{
	...
	switch (n) {
		...
		default:
			g_hal->log(LOG_ERROR, LOG_ERROR_UNIMPLEMENTED_IO_READ, n, pc);
	}
...
static void set_io(u12_t n, u4_t v)
{
	switch (n) {
		...
		default:
			g_hal->log(LOG_ERROR, LOG_ERROR_UNIMPLEMENTED_IO_WRITE, v, n, pc);
	}
}
...
static u4_t get_memory(u12_t n)
{
	...
	if (n < MEM_RAM_SIZE) {
		...
		g_hal->log(LOG_ERROR, LOG_ERROR_INVALID_MEMORY_READ, n, pc);
		return 0;
	}

	g_hal->log(LOG_MEMORY, LOG_MEMORY_DATA_READ, res, n, pc);
...
static void set_memory(u12_t n, u4_t v)
{
	...
	if (n < MEM_RAM_SIZE) {
		...
		g_hal->log(LOG_ERROR, LOG_ERROR_INVALID_MEMORY_WRITE, v, n, pc);
		return;
	}

	g_hal->log(LOG_MEMORY, LOG_MEMORY_DATA_WRITE, v, n, pc);
}
...
static void print_state(u8_t op_num, u12_t op, u13_t addr)
{
	...
	g_hal->log(LOG_CPU, LOG_CPU_STATS, pc, sp, np, x, y, a, b, flags);
}
...
int cpu_step(void)
{
	...
	if (!cpu_halted) {
		...
		if (ops[i].log == NULL) {
			g_hal->log(LOG_ERROR, LOG_ERROR_UNKNOWN_OPCODE, op, pc);
			return 1;
		}
